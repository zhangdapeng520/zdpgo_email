...
func newTestClient(t *testing.T) (c *EmailImap, s *serverConn) {
	return newTestClientWithGreeting(t, "* OK [CAPABILITY IMAP4rev1 STARTTLS AUTH=PLAIN UNSELECT] Server ready.\r\n")
}

func newTestClientWithGreeting(t *testing.T, greeting string) (c *EmailImap, s *serverConn) {
...
func setClientState(c *EmailImap, state imap.ConnState, mailbox *imap.MailboxStatus) {
	c.locker.Lock()
	c.state = state
	c.mailbox = mailbox
	c.locker.Unlock()
