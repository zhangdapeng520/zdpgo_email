...
func (c *EmailImap) executeSearch(uid bool, criteria *imap.SearchCriteria, charset string) (ids []uint32, status *imap.StatusResp, err error) {
	if c.State() != imap.SelectedState {
...
func (c *EmailImap) search(uid bool, criteria *imap.SearchCriteria) (ids []uint32, err error) {
...
func (c *EmailImap) fetch(uid bool, seqset *imap.SeqSet, items []imap.FetchItem, ch chan *imap.Message) error {
	defer close(ch)
...
func (c *EmailImap) store(uid bool, seqset *imap.SeqSet, item imap.StoreItem, value interface{}, ch chan *imap.Message) error {
	if ch != nil {
		defer close(ch)
	}
...
func (c *EmailImap) copy(uid bool, seqset *imap.SeqSet, dest string) error {
	if c.State() != imap.SelectedState {
		return ErrNoMailboxSelected
	}
...
func (c *EmailImap) move(uid bool, seqset *imap.SeqSet, dest string) error {
	if c.State() != imap.SelectedState {
		return ErrNoMailboxSelected
	}
...
// MOVE.
func (c *EmailImap) moveFallback(uid bool, seqset *imap.SeqSet, dest string) error {
