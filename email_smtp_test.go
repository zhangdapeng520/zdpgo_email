...
func basicTests(t *testing.T, e *EmailSmtp) *mail.Message {
	raw, err := e.Bytes()
	if err != nil {
...
func TestParseSender(t *testing.T) {
	var cases = []struct {
		...
		{
			EmailSmtp{From: "from@test.com"},
			"from@test.com",
			false,
		},
		{
			EmailSmtp{Sender: "sender@test.com", From: "from@test.com"},
			"sender@test.com",
			false,
		},
		{
			EmailSmtp{Sender: "bad_address_sender"},
			"",
			true,
		},
		{
			EmailSmtp{Sender: "good@sender.com", From: "bad_address_from"},
			"good@sender.com",
			false,
		},
	}
...
func TestEmail_SendHtmlAndAttach(t *testing.T) {
	email := prepareEmailSmtp()
	email.SendHtmlAndAttach("这是一封测试邮件", "<h1>这是一封HTML邮件</h1><div style='color:red'>疯狂的内容</div>", "README.md",
		"lxgzhw@163.com")
}
...
func TestEmail_SendTextAndAttach(t *testing.T) {
	email := prepareEmailSmtp()
	email.SendTextAndAttach("这是一封测试邮件", "<h1>这是一封HTML邮件</h1><div style='color:red'>疯狂的内容</div>", "README.md",
		"lxgzhw@163.com")
}
